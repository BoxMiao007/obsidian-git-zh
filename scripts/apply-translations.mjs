...
const TRANSLATIONS = {
    ...
    '"Integrate Git version control with automatic backup and other advanced features."':
        '"在 Obsidian.md 中集成 Git 版本控制，自动备份及其他高级功能。"',
    ...
    '"HTTP/HTTPS Proxy URL"': '"HTTP/HTTPS 代理 URL"',
    ...
    '"Custom base path (Git repository path)"':
        '"自定义基础路径（Git 仓库路径）"',
    "\"Custom Git directory path (Instead '.git')\"":
        "\"自定义 Git 目录路径（替代 '.git')\"",
    ...
    '"Push commits every X minutes. Set to 0 (default) to disable."':
        '"每 X 分钟推送提交。设置为 0（默认值）以禁用。"',
    '"Pull changes every X minutes. Set to 0 (default) to disable."':
        '"每 X 分钟拉取更改。设置为 0（默认值）以禁用。"',
    '"Automatically pull commits when Obsidian starts."':
        '"当 Obsidian 启动时自动拉取提交。"',
    '"Show the author of the commit in the history view."':
        '"在历史记录视图中显示提交的作者。"',
    '"Obsidian must be restarted for this change to take effect."':
        '"必须重新启动 Obsidian 才能使此更改生效。"',
    '"Show the current branch in the status bar."':
        '"在状态栏中显示当前分支。"',
    '"Recommended to use Personal Access Token instead of password. Not needed when SSH is set up."':
        '"建议使用个人访问令牌而不是密码。设置 SSH 时不需要。"',
    ...
    '"Disable informative notifications for git operations to minimize distraction (refer to status bar for updates)."':
        '"禁用 Git 操作的信息通知以减少干扰（请参阅状态栏以获取更新）。"',
    '"Disable error notifications of any kind to minimize distraction (refer to status bar for updates)."':
        '"禁用所有类型的错误通知以减少干扰（请参阅状态栏以获取更新）。"',
    "\"Disable notifications for 'No changes to commit/push' to minimize distraction.\"":
        '"禁用「没有更改可提交/推送」的通知以减少干扰。"',
    '"Add options to the file menu to stage, unstage and discard changes."':
        '"在文件菜单中添加暂存、取消暂存和放弃更改的选项。"',
    '"On slower machines this may cause lags. If so, just disable this option."':
        '"在较慢的机器上，这可能会导致卡顿。如果是这样，只需禁用此选项。"',
    '"Milliseconds to wait after file change before refreshing the Source Control View."':
        '"文件更改后等待刷新源代码管理视图的毫秒数。"',
    "\"A script that is run using 'sh -c' to generate the commit message. May be used to generate commit messages using AI tools. Available placeholders: {{hostname}}, {{date}}.\"":
        "\"使用 'sh -c' 运行的脚本，用于生成提交信息。可用于使用 AI 工具生成提交信息。可用占位符：{{hostname}}、{{date}}。\"",
    "\"Update submodules when pulling, pushing, and committing. This will run 'git submodule update --remote --merge' on pull, and 'git submodule foreach git push' on push.\"":
        "\"在拉取、推送和提交时更新子模块。这将在拉取时运行 'git submodule update --remote --merge'，在推送时运行 'git submodule foreach git push'。\"",
    '"When checking out or switching branches, recursively update submodules to match the state of the parent repository."':
        '"在检出或切换分支时，递归更新子模块以匹配父仓库的状态。"',
    '"Set a custom proxy for desktop native Git network operations, e.g. http://127.0.0.1:7890. Leave empty to disable."':
        '"为桌面端原生 Git 网络操作设置自定义代理，例如 http://127.0.0.1:7890。留空则禁用。"',
    '"Specify a custom path to the Git binary. This is useful if Git is not in the PATH."':
        '"指定 Git 二进制文件的自定义路径。如果 Git 不在 PATH 中，这很有用。"',
    '"Use each line for one environment variable in the format KEY=VALUE."':
        '"每行使用一个环境变量，格式为 KEY=VALUE。"',
    '"Use each line for one path"': '"每行使用一个路径"',
    '"Reload Obsidian Git with the new environment variables and/or PATH."':
        '"使用新的环境变量和/或 PATH 重新加载 Obsidian Git。"',
    '"Specify a custom base path for the Git repository. This is useful if the vault is not the root of the Git repository."':
        '"为 Git 仓库指定自定义基础路径。如果仓库不是 Git 仓库的根目录，这很有用。"',
    '"Specify a custom path to the Git directory (default: .git). This is useful for worktrees or special setups."':
        '"指定 Git 目录的自定义路径（默认：.git）。这对于工作树或特殊设置很有用。"',
    '"Set the style for the diff view. Note that the actual diff in \\"Split\\" mode is not generated by Git, but the editor itself instead so it may differ from the diff generated by Git. One advantage of this is that you can edit the text in that view."':
        '"设置差异视图的样式。请注意，「分屏」模式下的实际差异不是由 Git 生成的，而是由编辑器本身生成的，因此可能与 Git 生成的差异不同。这样做的一个优点是您可以在该视图中编辑文本。"',
    '"Show the date of the commit in the history view. The {{date}} placeholder format is used to display the date."':
        '"在历史记录视图中显示提交的日期。使用 {{date}} 占位符格式显示日期。"',
    '"Hunks are sections of grouped line changes right in your editor."':
        '"代码块是编辑器中分组行更改的部分。"',
    '"If and how the authoring date is displayed. The formats for relative and custom date formats can be configured below."':
        '"是否以及如何显示创作日期。相对日期和自定义日期格式的格式可以在下方配置。"',
    '"Specify custom date format. E.g. \\"YYYY-MM-DD HH:mm:ss\\". See Moment.js for more formats."':
        '"指定自定义日期格式。例如：\\"YYYY-MM-DD HH:mm:ss\\"。有关更多格式，请参阅 Moment.js。"',
    '"Available placeholders: {{date}} (see below), {{hostname}} (see below), {{numFiles}} (number of changed files in the commit) and {{files}} (changed files in commit message)."':
        '"可用占位符：{{date}}（见下文）、{{hostname}}（见下文）、{{numFiles}}（提交中更改的文件数量）和 {{files}}（提交信息中的更改文件）。"',
    '"Available placeholders: {{date}} (see below), {{hostname}} (see below), {{numFiles}} (number of changed files in the commit) and {{files}} (changed files in commit message). Leave empty to require manual input on each commit."':
        '"可用占位符：{{date}}（见下文）、{{hostname}}（见下文）、{{numFiles}}（提交中更改的文件数量）和 {{files}}（提交信息中的更改文件）。留空则每次提交时需要手动输入。"',
    ...
    '"Git: Error while loading line authoring feature."':
        '"Git：加载行作者功能时出错。"',
    '"Git: Attempted to track change of undefined filepath. Unforeseen situation."':
        '"Git：尝试跟踪未定义文件路径的更改。意外情况。"',
    '"Git: undefined lineAuthorInfoProvider. Unexpected situation."':
        '"Git：未定义的 lineAuthorInfoProvider。意外情况。"',
    '"Aborted, because the following files are too big"':
        '"已中止，因为以下文件太大"',
    '"Please remove them or add to .gitignore."':
        '"请删除它们或添加到 .gitignore。"',
    '"Adding task to the queue, commands = "':
        '"正在将任务添加到队列，命令 = "',
    '"A fatal exception occurred in a previous task, the queue has been purged"':
        '"上一个任务中发生了致命异常，队列已被清除"',
    '"Fatal exception, any as-yet un-started tasks run through this executor will not be attempted"':
        '"致命异常，此执行器中尚未启动的任务将不会被执行"',
    '"Stdout from commit message script is empty. Using default message."':
        '"提交信息脚本的标准输出为空。使用默认信息。"',
    ...
    '"Please resolve them and commit them using the commands `Git: Commit all changes` followed by `Git: Push`"':
        '"请解决冲突，然后使用命令「Git：提交所有更改」后跟「Git：推送」提交"',
    '"I strongly recommend to use \\"Source mode\\" for viewing the conflicted files. For simple conflicts, in each file listed above replace every occurrence of the following text blocks with the desired text."':
        '"我强烈建议使用「源码模式」查看冲突文件。对于简单冲突，在上面列出的每个文件中，将以下文本块的每次出现替换为所需的文本。"',
    '"Are you sure you want to DELETE the "': '"您确定要删除 "',
    '"Are you sure you want to discard ALL changes in "': '"您确定要放弃 "',
    '"They are deleted according to your Obsidian trash settting."':
        '"它们将根据您的 Obsidian 回收站设置被删除。"',
    '"Discard all "': '"放弃所有 "',
    '"Delete all "': '"删除所有 "',
    ...
    '"Commit-and-sync with default settings means staging everything -> committing -> pulling -> pushing. Ideally this is a single action that you do regularly to keep your local and remote repository in sync."':
        '"提交并同步的默认设置意味着暂存所有内容 -> 提交 -> 拉取 -> 推送。理想情况下，这是您定期执行的单个操作，用于保持本地和远程仓库同步。"',
...
const TEMPLATE_TRANSLATIONS = [
    ...
    },
    {
        ...
        replace:
            "每 X 分钟更改一次。设置为 0（默认值）以禁用。（请参阅下方的设置以进行进一步配置！）",
    },
    {
        ...
        replace:
            "如果启用，将上次自动${n}时间戳设置为最新提交时间戳。这可以减少手动提交时自动${n}的频率。",
    },
    {
        ...
        replace:
            "如果启用，则只有暂存的文件会在${n}时提交。如果禁用，所有更改的文件都会被提交。",
    },
    {
        // Committed message
        search: /`Committed\$\{[^}]+\} \$\{[^}]+\}\s*\$\{[^}]+\s*\? "file" : "files"\s*\}`/g,
        replace:
            '`已提交${roughly ? " 约" : ""} ${committedFiles} ${committedFiles == 1 ? "个文件" : "个文件"}`',
    },
    {
        // Pushed to remote
    ...
    },
    {
        ...
        replace:
            '`已推送 ${pushedFiles} ${pushedFiles == 1 ? "个文件" : "个文件"} 到远程`',
    },
    // ========== 从123.txt提取的模板字符串 ==========
    {
    ...
    },
    {
        ...
        replace:
            "提交哈希、作者名称和创作日期都可以单独切换。</br>隐藏所有内容，仅显示按年龄着色的侧边栏。",
    },
    {
    ...
    },
    {
        ...
        replace:
            '"提交并同步"和"拉取"会处理子模块。缺失的功能：冲突文件、已拉取/已推送/已提交文件的计数。每个子模块都需要设置跟踪分支。',
    },
    {
    ...
    },
    {
        ...
        replace:
            "这也可以防止在编辑文件时自动${n}。如果关闭，它与上次文件编辑无关。",
    },
    {
    ...
    },
    {
        ...
        replace:
            '将"暂存"、"取消暂存"和"添加到 .gitignore"操作添加到文件菜单。',
    },
    {
    ...
    },
    {
        ...
        replace:
            "您始终可以通过打开控制台来查看此插件和所有其他插件的日志，方法是",
    },
    {
        // for more formats.
